				...
				<div class="col-md-6">
					...
					<div class="card card-primary">
						<div class="card-header">
							<h3 class="card-title">Chỉnh sửa tên sản phẩm</h3>
						</div>
							...
							<div class="card-body">
								...
								<div class="form-group">
									<label>Danh mục sản phẩm</label> <select name="category" id="category"
										class="form-control"
										data-selected="${product.getCategory().getName()}" required>
										<!-- <option value="">Chọn 1 thể loại</option> -->
										<c:forEach items="${categoryList}" var="category">
											<option value="${Integer.toString(category.getId())}"
												<c:if test="${category.getId() == product.getCategory().getId()}">selected="true"</c:if>>${category.getName()}</option>
										</c:forEach>
									</select>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label for="exampleInputFile">Thay ảnh (Nếu cần)</label>
									<div class="input-group">
										<div class="custom-file">
											<input type="hidden" name="image_str"
												value="${product.getImage()}" /> <input type="file"
